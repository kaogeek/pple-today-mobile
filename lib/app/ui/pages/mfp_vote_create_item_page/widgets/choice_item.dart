...
class ChoiceItem extends GetWidget<MfpVoteCreateItemController> {
  const ChoiceItem({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Obx(() => (controller.createItemModel.value.voteItem ?? []).isNotEmpty && controller.createItemModel.value.voteItem![controller.index].typeChoice == 'text'
        ? Container()
        : Group(
            groupName: 'ตัวเลือก',
            child: Container(
              ...
              child: Obx(() => Column(
                    children: [
                      ...List.generate(
                        ...
                        // controller.createItemModel.value.voteItem![controller.index].voteChoice!.length + 1,
                        (index) {
                          ...
                          if (index + 1 != _voteChoice.length + 1) {
                            return TextField(
                              ...
                              autocorrect: false,
                              onChanged: (value) {
                                controller.voteChoiceModel[index].title = value.trim();
                                // controller.createItemModel.update((val) {
                                //   val!.voteItem![controller.index].voteChoice![index].title = value.trim();
                                // });
                              },
                            );
                          } else {
                            return TextButton(
                              onPressed: () {
                                ...
                                //   );
                                // });
                                // controller.createItemModel.value.voteItem![controller.index].voteChoice!.last.focusNode!.requestFocus();
                              },
                              child: const Text(
                                'เพิ่มตัวเลือก',
                                style: TextStyle(
