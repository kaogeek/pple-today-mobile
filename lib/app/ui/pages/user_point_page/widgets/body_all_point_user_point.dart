...
class BodyAllPointUserPoint extends GetWidget<UserPointController> {
  const BodyAllPointUserPoint({super.key});
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final totalPointStatement = controller.dataModel.value.data?.accumulatePoint?.totalPointStatement;
      if ((totalPointStatement ?? []).isEmpty) {
        return Center(
          child: Text(
            'ไม่มีข้อมูล',
            style: TextStyle(
      ...
      return ListView(
        ...
        children: ListTile.divideTiles(
            context: context,
            tiles: totalPointStatement!.map((e) {
              final bool losePoints = e.type == 'REDEEM' || e.type == 'USE_COUPON' || e.type == 'ADMIN_REDUCE_POINT';
                ...
                child: ListTile(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          Text(
                            losePoints ? '-${formatPoint(e.point!)}' : '+${formatPoint(e.point!)}',
                            style: TextStyle(
                              fontSize: context.isPhone ? 16 : 24,
                              fontFamily: Assets.assetsFontsAnakotmaiMedium,
                              color: losePoints ? kPrimaryColor : Colors.green,
                              overflow: TextOverflow.ellipsis,
                            ),
                          ),
                        ],
                  ...
                  subtitle: Row(
                    children: [
                      Expanded(
                        child: Text(
                          losePoints ? 'ได้ใช้คะแนน' : 'ได้รับคะแนน',
                          style: TextStyle(
